...
export abstract class IndexedEntity<S extends { id: string }> extends Entity<S> {
  ...
  protected static async createBase<T extends {id: string}>(env: Env, indexName: string, state: T): Promise<T> {
    // @ts-ignore
    const inst = new this(env, state.id);
    await inst.save(state);
    const idx = new Index<string>(env, indexName);
  ...
  protected static async listBase<T extends {id: string}>(env: Env, indexName: string): Promise<{ items: T[] }> {
    const idx = new Index<string>(env, indexName);
    const ids = await idx.list();
    // @ts-ignore
    const rows = (await Promise.all(ids.map((id) => new this(env, id).getState()))) as T[];
    return { items: rows };
  }
  ...
  protected static async deleteBase(env: Env, indexName: string, id: string): Promise<boolean> {
    // @ts-ignore
    const inst = new this(env, id);
    const existed = await inst.delete();
    const idx = new Index<string>(env, indexName);
